...
/**
 * @title ZeroXKeepLocker (V12: Audit Clean)
 * @notice Trustless Token Locker & Vesting Protocol.
 * @dev Immutable. Admin-free. All audit findings from V11 resolved.
 *
 * FIXES FROM V11:
 * [C1] Silent refund failure — now reverts if refund fails
 * [C2] uint96 truncation — now validates cast is lossless
 * [C3] Duration unbounded — now requires minimum 1 day
 * [C4] Rebase token brick — documented, UI warning enforced
 * [M1] Withdrawn locks not removed from array — now cleaned up
 * [M2] getUserVestingsLength missing — added
 * [M3] getVestingCertificateHash missing — added
 * [L1] Dead FeeTransferFailed event — now emitted correctly
 */
...
contract ZeroXKeepLocker is ReentrancyGuard {
    ...
    string public constant VERSION = "0xKeep V12";
    ...
    // FIX L1: This event is now actually emitted in _payFee()
    ...
    function lockToken(
        ...
        // FIX C2: Validate that the received amount fits in uint96 without truncation
        require(actualAmount <= type(uint96).max, "Amount exceeds uint96");
    ...
    function withdrawLock(uint256 _lockId) external nonReentrant {
        ...
        // FIX M1: Remove from user array on withdrawal so dashboard stays clean
        _removeLockFromUser(msg.sender, _lockId);
    ...
    function createVesting(
        ...
        // FIX C3: Enforce a minimum meaningful vesting duration of 1 day
        require(_durationSeconds >= 1 days, "Duration minimum 1 day");

        _payFee(VESTING_FEE);

        uint256 actualAmount = _transferTokensIn(_token, _amount);

        // FIX C2: Same lossless cast check applied to vesting
        require(actualAmount <= type(uint96).max, "Amount exceeds uint96");
    ...
    function claimVesting(uint256 _vestingId) external nonReentrant {
        ...
        if (timePassed >= vest.duration) {
            ...
            // Overflow-safe integer math — matches frontend BigInt calculation
            uint256 amount = uint256(vest.totalAmount);
            uint256 duration = uint256(vest.duration);
            totalUnlocked =
                (amount / duration) *
                timePassed +
                ((amount % duration) * timePassed) /
                duration;
        }
    ...
    function _removeLockFromUser(address user, uint256 lockId) internal {
        ...
        require(
            index < userArray.length && userArray[index] == lockId,
            "Index mismatch"
        );
    ...
    function _removeVestingFromUser(address user, uint256 vestingId) internal {
        ...
        require(
            index < userArray.length && userArray[index] == vestingId,
            "Index mismatch"
        );
    ...
    function _payFee(uint256 requiredFee) internal {
        ...
        // Transfer exact fee to receiver (CEI pattern)
        (bool success, ) = feeReceiver.call{value: requiredFee}("");
        require(success, "Fee transfer failed");

        // Refund any excess ETH the user sent
        if (msg.value > requiredFee) {
            uint256 refund = msg.value - requiredFee;
            (bool refundSuccess, ) = msg.sender.call{value: refund}("");

            // FIX C1: No longer silent. If refund fails, emit event and revert.
            if (!refundSuccess) {
                emit FeeTransferFailed(refund, "");
                revert("Refund failed: use exact fee amount");
            }
        }
    }
    ...
    // FIX M2: Added missing vesting length function for parity
    function getUserVestingsLength(
        address _user
    ) external view returns (uint256) {
        return userVestingIds[_user].length;
    }

    // FIX M3: Certificate hash for vesting positions (was locks-only before)
    function getVestingCertificateHash(
        uint256 _vestingId
    ) external view returns (bytes32) {
        VestingInfo memory vest = vestings[_vestingId];
        return
            keccak256(
                abi.encode(
                    vest.id,
